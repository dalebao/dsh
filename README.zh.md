...
### 4. ExecPlan 创建由 Skill 门控

原始规则是"如果任务模糊才写 ExecPlan"。这是个错误——它把判断权交给了 agent。agent 永远选择"不模糊"然后跳过计划。

修正后的规则：**ExecPlan 仅在用户明确要求时创建。** 只有两条路：

```
/plan 或 "创建 ExecPlan" → execplan-create skill → PLAN → PENDING → 批准 → EXECUTE
其他请求                → 轻量讨论（不创建 ExecPlan）
```

没有猜测。没有"这事太小不用计划"。agent 不决定什么该计划。
...
| 理念 | 文件 | 实现机制 |
|------|------|---------|
| 状态机 | `AGENTS.md` | 11 条 MUST 约束 + 状态转移表 + `[STATE]` 头 |
| 可检查约束 | `.harness/constraints.yaml` | 10 个不变量 + 2 个防护栏，每个有 `check` 字段 |
| 提案/执行分离 | `AGENTS.md` 约束 8 | "单个回复不得同时包含新提案和它的实现" |
| Skill 门控 | `execplan-create.md` | 仅 /plan 或明确要求时创建 ExecPlan |
| 纵深防御 | `.harness/docs/L4_SKILLS/hooks.md` | 预提交钩子设计 + 验证流水线 |
...
初始版本。本次开发会话的关键设计决策和变更：

- **ExecPlan 生命周期**：引入 Phase-Annotated 模板，`proposal/ → active/ → completed/` 三层目录。Section 从 ~11 重组成 6 个 Phase（Plan → Review Gate → Execution → Audit → Close）。
- **触发模型**：从"每个请求都是 ExecPlan"改为 skill 门控创建（仅 `/plan` 或用户明确要求时）。agent 不再猜测用户意图。
- **配置清理**：
  - `constraints.yaml`：28→10 个不变量（-64%）。移除无验证回路的规则，仅保留核心工作流约束。
  - `config.yaml`：删除 `gc`、`L1_language`、`auto_block_on`/`auto_flag_for_review` — 有定义无实现的装饰字段。
  - `taxonomy.yaml`：删除（无采集机制）。
  - `metrics/definitions.yaml`：移至 `.harness/docs/L3_FACTS/metrics-design.md` 作为设计参考。
  - 新增 `validate-yaml.sh` 验证脚本，集成到 `verification.layers`。
- **L4_SKILLS 清理**：13→10 个文件，780→356 行。删除 `doc-sync.md`（逻辑在 config.yaml）、`goal-check.md`（零次执行）、`code-index.md`（含运行时代码项目的设计文档）。修剪 `checkpoint.md`、`report.md`、`hooks.md`、`verify.md`、`decision-log.md`。
- **template/AGENTS.md**：从根 AGENTS.md 分离。模板版本使用通用 CORE GOAL 占位符，移除项目特定约束（#7, #8）。
- **版本**：0.1.0 — 稳定基线。ExecPlan 模板格式无破坏性变更。所有文件向前兼容。
